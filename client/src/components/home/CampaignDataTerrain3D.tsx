...
export default function CampaignDataTerrain3D() {
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [hoveredNode, setHoveredNode] = useState<typeof funnelNodes3D[0] | null>(null);
  ...
  useEffect(() => {
    ...
    // Add path (line)
    const curve = new THREE.CatmullRomCurve3(funnelNodes3D.map(n => n.pos));
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        const nodeData = object.userData as typeof funnelNodes3D[0];
